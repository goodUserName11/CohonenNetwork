...
namespace CohonenMaps
{
    ...
    internal class NeuralNetwork
    {
        ...
        public NeuralNetwork(int width, int height, int inputDimension, int numberOfIterations, double learningRate)
        {
            _width = width;
            _height = height;
            _matrix = new Neuron[_width, _height];
        ...
        internal (int xStart, int xEnd, int yStart, int yEnd) GetRadiusIndexes(Neuron bmu, double currentRadius)
        {
        ...
        internal Neuron GetNeuron(int indexX, int indexY)
        {
        ...
        internal Neuron CalculateBMU(Vector input)
        {
            Neuron bmu = _matrix[0, 0];
            double bestDist = input.Distance(bmu.Weights);

            for (int i = 0; i < _width; i++)
            {
                for (int j = 0; j < _height; j++)
                {
                    var distance = input.Distance(_matrix[i, j].Weights);
